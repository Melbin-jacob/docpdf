...
export default function ProtectPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const modified = await protectPDF(new Uint8Array(buffer), password);
      setProcessedData(modified);
      if (localStorage.getItem("ad_enabled") === "true") {
        setIsAdOpen(true);
      } else {
        triggerDownload(modified, "protected.pdf");
      }
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-4">
      <h1 className="text-3xl font-bold mb-6 text-center">Protect PDF</h1>
      {!file ? (
        ...
        <div className="bg-white p-6 border rounded-lg shadow-sm">
          ...
          <div className="mb-6">
            <label className="block text-sm font-medium mb-2 text-gray-700">Set Password:</label>
            <input
              type="password"
              value={password}
          ...
          >
            {processing ? <Loader2 className="animate-spin" /> : <><Lock className="w-5 h-5" /> <span>Protect and Download</span></>}
          </button>
        </div>
      )}
      <AdTimer isOpen={isAdOpen} onComplete={() => { setIsAdOpen(false); if(processedData) triggerDownload(processedData, "protected.pdf"); }} />
    </div>
  );
}
